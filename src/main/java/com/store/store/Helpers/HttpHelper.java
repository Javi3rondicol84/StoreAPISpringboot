...
public class HttpHelper {
    ...
    public ResponseEntity<?> getPutResponse(ItemEntity item, Long id) {
        ...
        if (item == null) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("El item en el id: " + id + " no se pudo actualizar");
        } else if (id == null) {
    ...
    public ResponseEntity<?> getItemsByCategoryResponse(List<? extends ItemEntity> items, String category) {
        ...
        if (items == null) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("No se pudo encontrar items con la categoria: " + category);
        } else {
    ...
    public ResponseEntity<?> getItemsByPriceResponse(List<? extends ItemEntity> items, Double price) {
        ...
        if (items == null) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                    .body("No se pudo encontrar items con el precio: " + price);
        } else {
            return ResponseEntity.status(HttpStatus.ACCEPTED).body(items);
        }
    ...
    public ResponseEntity<?> getAllCategoriesResponse(List<String> items) {
        if (items == null) {
    ...
    public ResponseEntity<?> getItemsByKeyWordResponse(List<ProductEntity> items, String keyWord) {
        if (items == null) {
    ...
    public ResponseEntity<?> getByCategoryResponse(List<ProductEntity> items, String category) {
        if (items == null) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST)
